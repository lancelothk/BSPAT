...
public class DrawComparisonGraph extends DrawGraph {

	public DrawComparisonGraph(ArrayList<ReadAnalysisResult> dataList) throws IOException {
		super(dataList);
	}
	...
	protected void addItems(GScene scene, ReadAnalysisResult data) {
		...
		for (CpGStatistics cpgStat : statList) {
			if (cpgStat.getPosition() != 0) {
				...
				GObject gCpG = new GCpGSite(startX + (cpgStat.getPosition() * WIDTH), height, new Color(R, G, B),
						RADIUS);
				scene.add(gCpG);
		...
		GMyText statText = new GMyText((refLength + 10) * WIDTH + startX, height, "Average", 35);
		scene.add(statText);
